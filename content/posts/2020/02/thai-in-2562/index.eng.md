...
In Southeast Asia, Buddhism takes the same place as Christianity in Europe and America. So, instead of the [Gregorian calendar], which uses the birth of Jesus Christ as an epoch, Southeast Asia generally uses the [Buddhist calendar] which takes Buddha’s attainment of Parinirvana(nirvana after death) as an epoch. Thailand is no exception, so it uses the [Thai solar calendar], which is a solar modification of the traditional Buddhist lunar calendar. Buddha’s Parinirvana was in 543 BC, and so the year 2019 A.D. becomes 2562 under the Thai solar calendar.
